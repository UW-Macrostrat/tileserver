...
function makeLayer(scale, cartoCSS, callback) {
  ...
  let burwell = {
    ...
    "Layer": [{
      ...
      "Datasource": {
          "type": "postgis",
          "table": `(SELECT z.map_id, COALESCE(l.color, '#777777') AS color, z.geom FROM carto_new.${scale} z LEFT JOIN maps.map_legend ON z.map_id = map_legend.map_id LEFT JOIN maps.legend AS l ON l.legend_id = map_legend.legend_id LEFT JOIN maps.sources ON l.source_id = sources.source_id WHERE sources.status_code = 'active') subset`,
          "key_field": "map_id",
          "geometry_field": "geom",
          "extent_cache": "auto",
    ...
    "Datasource": {
        "type": "postgis",
        "table": `(SELECT x.line_id, x.geom, q.direction, q.type FROM carto_new.lines_${scale} x LEFT JOIN ( ${lineSQLJoin} ) q on q.line_id = x.line_id LEFT JOIN maps.sources ON x.source_id = sources.source_id WHERE sources.status_code = 'active') subset`,
        "key_field": "line_id",
        "geometry_field": "geom",
        "extent_cache": "auto",
        "extent": "-179,-89,179,89",
  ...
  let mapnikXML = new carto.Renderer({
    //paths: [ __dirname ],
    filename: 'temp.mml',
    local_data_dir: path.dirname('temp.mml')
  }).render(burwell)
